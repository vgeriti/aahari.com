...
if(!isset($_SESSION['robo']))
{
	if(isset($_POST))
	{
		...
		$st = $db->prepare("SELECT email,name from users where email=?");
		$st->execute(array($_REQUEST['reg_email']));
//echo "*".$st->rowCount()."*";
		if($st->rowCount() < 1)
		{
			$st = $db->prepare("INSERT into users(name,email,password) values(:name,:email,:password)");
			$result = $st->execute(array(':name'=>$_REQUEST['reg_name'],':email'=>$_REQUEST['reg_email'],':password'=>sha1($_REQUEST['reg_paswd'])));

			if($result)
			{
				//sendemail($_REQUEST['reg_name'],$_REQUEST['reg_email']);
...
else
{
	...
	$response['message'] = "Something went wrong with your request. We will work on it.";
}
...
{
	echo "in send email";
	require_once 'manag/php/classes/Mailer.php';
	$mailer = new Mailer();
	$mailer->setTo($email);
	$mailer->setFrom('Info@aahari.com');
	...
	$mailer->setBody('
		...
		</tr><tr><td>&nbsp;</td></tr><tr><td><table width="100%" border="0" cellspacing="0" cellpadding="0"><tbody><tr><td width="10%">&nbsp;</td><td width="80%" align="left" valign="top"><p><font style="font-family:Georgia,\'Times New Roman\',Times,serif;color:rgb(1,1,1);font-size:24px"><strong><em>Hi '.$name.',</em></strong></font><br>
