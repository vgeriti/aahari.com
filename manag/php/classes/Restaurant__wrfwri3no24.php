...
class Restaurant{
	...
	function getRestaurant($url){
		...
		try{
			...
			$stmt = $con->prepare("SELECT * from restaurants where rest_url =?");
	...
	function saveRestaurant($data,$rest_status)
	{
		...
		echo "@#$ ".$data[':rest_url']." @#$";
		$ary1 = array(':rest_url' => $this->createRestURL($data[':name']));
		
		if($rest_status =='n' && !isset($data[':id']))
		{
			$data = $ary1+$data;
			echo "inserting new one";
			$stmt = $con->prepare("INSERT into restaurants(name, rest_url, area_id, doorNo, landmark, pincode, contact1, contact2, contact3, email, feed_email, features, services, closedOn, deliver_distance, edeliverytime, start_time, end_time, happyh_start, happyh_end, aboutus,latitude, longitude, servicecharge, vat_tax, mindeliveryamount, deliveryCharges, payment, time, useragent, ipaddr, inserted) VALUES 
														 (:name, :rest_url, :area_id, :doorNo, :landmark, :pincode, :contact1, :contact2, :contact3, :email, :feed_email, :features, :services, :closedOn, :deliver_distance, :edeliverytime, :start_time, :end_time, :happyh_start, :happyh_end, :aboutus, :latitude, :longitude, :servicecharge, :vat_tax, :mindeliveryamount, :deliveryCharges, :payment, :time, :useragent, :ipaddr, :inserted)");
			echo "^^ ".array_pop($data);	
		}
