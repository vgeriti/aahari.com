...
if(isset($_POST['rev_name']) && isset($_POST['rev_message']))
{
	...
	$var = $stmt->execute(array('restaurant_id'=>$_POST['rev_rest_id'],':title'=>$_POST['rev_name'],':review'=>$_POST['rev_message'],':rating'=>$_POST['rateng'],':dtime'=>date('Y-m-d H:i:s'),':user_id'=>$rating,':useragent'=>$_SERVER['HTTP_USER_AGENT']." REMOTE_ADDR:".$_SERVER['REMOTE_ADDR']))or die(print_r($stmt->errorInfo(),true));
