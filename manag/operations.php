	...
	<div class="wraper clearfix">
		<div class="side_bar">
			<ul class="nav nav-list">
				...
				<li><a href="operations.php?_aXTL=categories&_id=new&name=<?php echo $_REQUEST['name']?>">Categories</a></li>
			</ul>
		</div>
		<div class="main_body">
...
if(isset($_REQUEST['_aXTL']) && $_REQUEST['_aXTL']=="restaurant"){
	...
	<form class="form-horizontal" id="restaurantFrm" name="restaurantFrm">
		...
		<div class="control-group">
			<fieldset>
				...
				<label class="control-label">Service / VAT Tax</label>
				<div class="controls">
					<input type="text" id="hd_ResServiceTax" name="hd_ResServiceTax" placeholder="Service Tax" value="<?php echo $rest->getServiceTax(Aahari_Constants::HD)?>"> / 
					<input type="text" id="hd_ResVatTax" name="hd_ResVatTax" placeholder="Vat Tax" value="<?php echo $rest->getVatTax(Aahari_Constants::HD)?>">
				</div>
				<label class="control-label" for="hd_ResDiscount">Discount</label>
				<div class="controls">
					<input type="text" id="hd_ResDiscount" name="hd_ResDiscount" placeholder="Discount" value="<?php echo $rest->getDiscount(Aahari_Constants::HD)?>">
				</div>
			</fieldset>
		</div>
		...
		<div class="control-group">
			<fieldset>
				...
				<label class="control-label">Service / VAT Tax</label>
				<div class="controls">
					<input type="text" id="ta_ResServiceTax" name="ta_ResServiceTax" placeholder="Service Tax" value="<?php echo $rest->getServiceTax(Aahari_Constants::TA)?>"> / 
					<input type="text" id="ta_ResVatTax" name="ta_ResVatTax" placeholder="Vat Tax" value="<?php echo $rest->getVatTax(Aahari_Constants::TA)?>">
				</div>
				<label class="control-label" for="ta_ResDiscount">Discount</label>
				<div class="controls">
					<input type="text" id="ta_ResDiscount" name="ta_ResDiscount" placeholder="Discount" value="<?php echo $rest->getDiscount(Aahari_Constants::HD)?>">
				</div>
			</fieldset>
		</div>
		...
		<div class="control-group">
			<fieldset>
				...
				<label class="control-label">Service / VAT Tax</label>
				<div class="controls">
					<input type="text" id="tb_ResServiceTax" name="tb_ResServiceTax" placeholder="Service Tax" value="<?php echo $rest->getServiceTax(Aahari_Constants::TB)?>"> / 
					<input type="text" id="tb_ResVatTax" name="tb_ResVatTax" placeholder="Vat Tax" value="<?php echo $rest->getVatTax(Aahari_Constants::TB)?>">
				</div>
				<label class="control-label" for="tb_ResDiscount">Discount</label>
				<div class="controls">
					<input type="text" id="tb_ResDiscount" name="tb_ResDiscount" placeholder="Discount" value="<?php echo $rest->getDiscount(Aahari_Constants::HD)?>">
				</div>
			</fieldset>
		</div>
	...
	{
	require_once 'php/classes/Dishes.php';
	$rest_name=$_REQUEST['name'];
	$dsh = new Dishes();
	$rst_id = $dsh->getRestIdByURL($rest_name);	
	if($_REQUEST['_id'] != "new")
	{
	...
	else
	{
		...
		echo "<input type='hidden' name='rest_id' value='".$dsh->getRestIdByURL($rest_name)."' />";
		}
		else
		{
        ...
        <div class="control-group">
			...
			<div class="controls">
				<select name="dishCategory" id="dishCategory" class="form-control">
					<option value='' selected="selected" disabled="disabled" >choose category</option>
                <?php 				
				//$dsh->getCuisines($_REQUEST['name'], $_REQUEST['_id']);
				$dsh->getDishCat($_REQUEST['_id'],$rst_id);
	...
	$name=$_REQUEST['name'];
	$stmt = $db->prepare("SELECT r.id as restaurant_id, rest_url, c.name as cuisine,r.name as restaurant,d.name as name, d.veg as dishType, d.id as id,d.price as price from dishes d,restaurants r,cuisines c where d.restaurant_id=r.id and d.cuisine_id=c.id and r.rest_url=?");
	//echo $stmt->rowcount();
	//str_replace('-',' ',$name)
...
if(isset($_REQUEST['_aXTL']) && $_REQUEST['_aXTL']=="categories")
{ ?>
	<br/><br/>
	<?php
	require_once 'php/classes/Dishes.php';
	$stmt = $db->prepare("SELECT cat.id as id, r.id as rest_id, cat.name as category from dish_category cat,restaurants r where cat.restaurant_id=r.id and cat.id=?");
	//echo $stmt->rowcount();
	$stmt->execute(array($_REQUEST['_id']));
	$ft = $stmt->fetch();
	$cat_name = $ft['category'];
	$cat_id = $ft['id'];
	$rest_id = $ft['rest_id'];
	$dsh = new Dishes();
	//echo $rest_id."################";
//str_replace('-',' ',$_REQUEST['name'])
	if(empty($rest_id))
		$rest_id = $dsh->getRestIdByURL($_REQUEST['name']);
	?>
<form class="form-horizontal catFrm" id="catFrm" name="catFrm">
	<div class="control-group">
			<label class="control-label" for="catName">Category</label>
			<div class="controls">
				<input type="text" id="catName" name="catName" placeholder="Enter Category" value="<?php echo $cat_name ?>">
			</div>
	</div>
			<input type="hidden" name="catId" id="catId" value="<?php echo $cat_id?>"/>
			<input type="hidden" name="restId" id="restId" value="<?php echo $rest_id?>"/>
	<div class="control-group">
			<div class="controls">
				<button type="submit" class="btn">Save</button>
			</div>
...
<table class="table table-hover">
	<tr><th>S.No</th><th>Category</th></tr>
	<?php
	$name=$_REQUEST['name'];
	$stmt = $db->prepare("SELECT cat.id as id, cat.name as category from dish_category cat,restaurants r where cat.restaurant_id=r.id and r.rest_url=?");
	//echo $stmt->rowcount();
	//str_replace('-',' ',$name)
	$stmt->execute(array($name));
	while($row=$stmt->fetch())
	{
		//$name=$row['category'];
	echo "<tr><td>".$row['id']."</td><td><a href='operations.php?_aXTL=categories&_id=".$row['id']."&name=".$_REQUEST['name']."'>".$row['category']."</a></td></tr>";
	}
?>
</table>	
